def make_player_df_from_playdf(playdf):
    ...
    player_df = play_df[["ホームアウェイF", "選手背番号", "ポジションID", "選手名"]].dropna().drop_duplicates().sort_values(["ホームアウェイF", "ポジションID"])
...
def norm_xy_to_gfootball(x, y):
    ...
    # 線形変換
    norm_x = ((x - datastudium_x_min) / (datastudium_x_max - datastudium_x_min)) * (gfootball_x_max - gfootball_x_min) + gfootball_x_min
