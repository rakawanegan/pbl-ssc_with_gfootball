...
class ShootDetector:
    ...
    def update(self, current_obs: dict) -> bool:
        ...
        if is_shoot:
            ...
            self.last_shoot_info = {
                "coordinates": np.array(current_obs["ball"]).tolist(),
                "direction": np.array(current_obs["ball_direction"]).tolist(),
            }
        else:
    ...
    def _detect_shoot(self, current_obs: dict, prev_obs: dict) -> bool:
        """
    ...
    def dump_shoot_info(self) -> dict:
        ...
        Returns:
            dict: シュートが打たれた際のボールの座標 [x, y, z] および方向 [vx, vy, vz] の辞書である。
                  例: {'coordinates': [x, y, z], 'direction': [vx, vy, vz]}
