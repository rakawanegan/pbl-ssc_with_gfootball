...
def main(cfg):
    ...
    # 特定フレームのデータ抽出
    target_frame = int(
        tracking_df.loc[tracking_df["No"] == 0, "Frame"].sample(1).iloc[0]
    )
    ...
    print(
        f"[info] Results of Winning rate: {results.count('Home') / max_iter * 100:.2f}% Home, {results.count('Away') / max_iter * 100:.2f}% Away, {results.count('Draw') / max_iter * 100:.2f}% Draw"
    )
